...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        target_dist = 0
        now_place = self.place
        while target_dist <= self.max_range:
            if now_place.bees == []:
                now_place = now_place.entrance
            else:
                return random_or_none(now_place.bees)
...
# BEGIN Problem 5A
class WallAnt(Ant):
    """A WallAnt does nothing each turn. It has a large armor value."""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
class ScubaThrower(ThrowerAnt):
    """water safe, otherwise identical from base class"""
    food_cost = 5
    watersafe = True
    implemented = True
    ...
    def action(self, colony):
        ...
        if not self.true_queen:
            ...
            while now_place != None:
                if now_place.ant:
                    if now_place.ant.container and now_place.ant.ant and not now_place.ant.ant.boosted:
                        now_place.ant.ant.damage *= 2
                        now_place.ant.ant.boosted = True
                    if now_place.ant.boosted == False:
                        now_place.ant.damage = now_place.ant.damage * 2
                        now_place.ant.boosted = True
