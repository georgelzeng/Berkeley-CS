...
class Graph(object):
    ...
    def get_inputs(self, node):
        ...
        """
        "*** YOUR CODE HERE ***"
        return [self.nodeValues[n] for n in node.get_parents()]
    ...
    def add(self, node):
        ...
        self.nodeValues[node] = node.forward(self.get_inputs(node))
        self.nodeGradients[node] = np.zeros_like(self.nodeValues[node])
    ...
    def backprop(self):
        ...
        for n in self.allNodes[-1::-1]:
            g = n.backward(self.get_inputs(n), self.get_gradient(n))
            for i in range(len(self.get_inputs(n))):
                self.nodeGradients[n.get_parents()[i]] += g[i]
    ...
    def step(self, step_size):
        ...
        """
        "*** YOUR CODE HERE ***"
        for v in self.inputVariables:
            v.data -= self.nodeGradients[v]*step_size
...
class Add(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [np.full(inputs[0].shape, gradient),
                np.full(inputs[1].shape, gradient)]
...
class ReLU(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [(np.maximum(inputs[0], 0)/inputs[0]) * gradient]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        numEntries = (inputs[0].shape)[0] * (inputs[0].shape)[1]
        return [(inputs[0]-inputs[1])*gradient/numEntries,
                (inputs[1]-inputs[0])*gradient/numEntries]
